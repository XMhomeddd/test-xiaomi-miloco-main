...
const [form] = Form.useForm();
  ...
  const [triggerPeriodStartHour, setTriggerPeriodStartHour] = useState(0);
  const [triggerPeriodStartMinute, setTriggerPeriodStartMinute] = useState(0);
  const [triggerPeriodEndHour, setTriggerPeriodEndHour] = useState(23);
  const [triggerPeriodEndMinute, setTriggerPeriodEndMinute] = useState(59);
...
useEffect(() => {
    ...
    if (mode !== 'create' && formData) {
      ...
      if (formData.filter) {
        ...
        setTriggerPeriodStartHour(filterData.triggerPeriodStartHour ?? 0);
        setTriggerPeriodStartMinute(filterData.triggerPeriodStartMinute ?? 0);
        setTriggerPeriodEndHour(filterData.triggerPeriodEndHour ?? 23);
        setTriggerPeriodEndMinute(filterData.triggerPeriodEndMinute ?? 59);
        ...
        if (filterData.triggerPeriod === 'custom') {
  ...
  const triggerPeriodOptions = useMemo(() => [
    { label: t('smartCenter.triggerPeriodAllDay'), value: 'all_day' },
    { label: t('smartCenter.triggerPeriodDaytime'), value: 'daytime' },
    { label: t('smartCenter.triggerPeriodNighttime'), value: 'nighttime' },
    { label: t('smartCenter.triggerPeriodCustom'), value: 'custom' },
  ], [t]);
          ...
          <div className={styles.advancedOptionsContent}>
            <div className={styles.advancedOptionItem}>
              ...
              <Select
                ...
                value={triggerPeriod}
                onChange={(value) => setTriggerPeriod(value || 'all_day')}
              ...
              {triggerPeriod === 'custom' && (
                <div className={styles.customPeriodSection}>
                  <div className={styles.triggerIntervalDescription}>
                    {t('smartCenter.triggerPeriodCustomDescription')}
                  </div>
                  <div className={styles.customPeriodRow}>
                    <span className={styles.customPeriodLabel}>{t('smartCenter.triggerPeriodStart')}</span>
                    <Select
                      value={triggerPeriodStartHour}
                      onChange={setTriggerPeriodStartHour}
                      options={TRIGGER_PERIOD_TIME_OPTIONS.hours}
                      className={styles.customPeriodTimePart}
                      disabled={isSubmitDisabled}
                    />
                    <span className={styles.customPeriodColon}>:</span>
                    <Select
                      value={triggerPeriodStartMinute}
                      onChange={setTriggerPeriodStartMinute}
                      options={TRIGGER_PERIOD_TIME_OPTIONS.minutes}
                      className={styles.customPeriodTimePart}
                      disabled={isSubmitDisabled}
                    />
                  </div>
                  <div className={styles.customPeriodRow}>
                    <span className={styles.customPeriodLabel}>{t('smartCenter.triggerPeriodEnd')}</span>
                    <Select
                      value={triggerPeriodEndHour}
                      onChange={setTriggerPeriodEndHour}
                      options={TRIGGER_PERIOD_TIME_OPTIONS.hours}
                      className={styles.customPeriodTimePart}
                      disabled={isSubmitDisabled}
                    />
                    <span className={styles.customPeriodColon}>:</span>
                    <Select
                      value={triggerPeriodEndMinute}
                      onChange={setTriggerPeriodEndMinute}
                      options={TRIGGER_PERIOD_TIME_OPTIONS.minutes}
                      className={styles.customPeriodTimePart}
                      disabled={isSubmitDisabled}
                    />
